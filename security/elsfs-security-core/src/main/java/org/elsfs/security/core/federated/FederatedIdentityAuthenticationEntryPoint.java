...
/**
 * {@link AuthenticationEntryPoint}用于启动到 使用{@code idp} 查询参数，它表示 {@code registrationId} 所需的
...
public final class FederatedIdentityAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response,
        ...
        var idp = request.getParameter("idp");
        if (idp != null) {
            var clientRegistration = this.clientRegistrationRepository.findByRegistrationId(idp);
            if (clientRegistration != null) {
                var redirectUri = UriComponentsBuilder.fromHttpRequest(new ServletServerHttpRequest(request))
                    .replaceQuery(null)
